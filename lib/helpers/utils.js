...
export async function parseCargoTomlData(
    ...
    // parse `[package]`
    if (
        packageNode instanceof Object && !Array.isArray(packageNode)
    ) {
        /** @type {Object} */
    ...
    if (dependenciesNode) {
        for (const dependencyName in dependenciesNode) {
            ...
            let version = "";
            if (
                typeof dependencyNode === "string" || dependencyNode instanceof String
            ) {
                // like `libc = 0.2.79`
                version = dependencyNode;
            } else if (
                Object.keys(dependencyNode).length > 0
            ) {
...
export function parseCargoDependencyData(cargoLockData) {
    ...
    const packageNode = cargoData?.package;
    if (
        !packageNode || !Array.isArray(packageNode)
    ) {
        return [];
    }
    /** @type {Array<Object>} */
    ...
    Object.values(pkgMap).forEach((pkg) => {
        ...
        pkg.dependencies?.forEach((dep) => {
            if (dep.indexOf(" ") !== -1) {
                ...
                dependsOn.add(
                    purlFromPackageInfo({name: depSplit[0].trim(), version: depSplit[1].trim()}),
                );
            } else if (pkgMap[dep]) {
